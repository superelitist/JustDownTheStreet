...
namespace JustDownTheStreet
{
  ...
  {
    public static Vehicle _personalVehicle;
    ...
    private static List<VehicleDefinition> PersonalVehiclesMichael = new List<VehicleDefinition>(JsonController.GetAllPersonalVehiclesFromJson("Michael"));
    private static List<VehicleDefinition> PersonalVehiclesFranklin = new List<VehicleDefinition>( JsonController.GetAllPersonalVehiclesFromJson( "Franklin" ) );
    ...
    internal static void UpdateHandler( String playerName, bool playerIsInControl) {
      ...
      if ( _personalVehicle != null && _deliveryIsInProgress && World.GetDistance( _personalVehicle.Position, Game.Player.Character.Position ) <= 15.0f && _personalVehicle.Speed < 10.0f ) {
        //_personalDriver.Delete();
        //_personalVehicle.PreviouslyOwnedByPlayer = true;
        Main.MakePlayerLookAtTarget(_personalDriver, 3333);
        _deliveryIsInProgress = false;
        _parkingIsInProgress = ParkPersonalVehicleNearby(true);
      ...
      if (_personalVehicle != null && _personalDriver != null && _parkingIsInProgress && _personalVehicle.Speed < 0.5f && World.GetDistance(_personalDriver.Position, Game.Player.Character.Position) <= 7.0f) {
        _parkingIsInProgress = ParkPersonalVehicleNearby(false);
      }
    }
  }
}
